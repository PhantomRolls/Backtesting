...
class StrategyApp:
    ...
    def open_pairs_trading_settings(self):
        ...
        def save():
            try:
                ...
                self.save_json_config(self.buy_and_hold_config, "pairs_trading.json")
                window.destroy()
            except ValueError:
                tk.messagebox.showerror("Erreur", "Entrée invalide")

        ttk.Label(window, text="Fenêtre de calcule du Z-score").grid(row=0, column=0, sticky='w', padx=10, pady=5)
        ttk.Entry(window, textvariable=spread_window).grid(row=0, column=1, padx=10)

        ttk.Label(window, text="Seuil d'entrée").grid(row=1, column=0, sticky='w', padx=10, pady=5)
        ttk.Entry(window, textvariable=z_enter).grid(row=1, column=1, padx=10)
        
        ttk.Label(window, text="Seuil de sortie").grid(row=2, column=0, sticky='w', padx=10, pady=5)
    ...
    def open_markowitz_settings(self):
        ...
        ttk.Label(window, text="Fenêtre d'opitmisation").grid(row=0, column=0, sticky='w', padx=10, pady=5)
        ttk.Entry(window, textvariable=lookback_window).grid(row=0, column=1, padx=10)

        ttk.Label(window, text="Fenêtre de rééqulibrage").grid(row=1, column=0, sticky='w', padx=10, pady=5)
